// chart.js - All chart functionality isolated here with fixes
...
// --- Lookahead vertical line overlay (visual only) ---
let lookaheadOverlay = null;
let lookaheadOffsetPx = 0;
let lookaheadBarsAhead = 1;
let updateLookaheadDebounceTimer = null;
...
/**
 * Create the absolute-positioned vertical overlay line element and attach it
 * to the chart container. Call once after chart is created.
 */
function createLookAheadOverlay() {
  if (!activeChart) {
    console.warn("Cannot create lookahead overlay: activeChart is null");
    return;
  }
  
  const container = document.getElementById('tradingview-widget');
  if (!container) {
    console.error("Cannot create lookahead overlay: container not found");
    return;
  }
  
  // Remove if exists
  if (lookaheadOverlay && lookaheadOverlay.parentNode) {
    lookaheadOverlay.parentNode.removeChild(lookaheadOverlay);
  }

  lookaheadOverlay = document.createElement('div');
  lookaheadOverlay.id = 'lookahead-line';
  Object.assign(lookaheadOverlay.style, {
    position: 'absolute',
    top: '0',
    bottom: '0',
    width: '2px',
  ...
  container.appendChild(lookaheadOverlay);
  console.log("Lookahead overlay created successfully");
  ...
  new ResizeObserver(() => {
    debouncedUpdateLookahead();
  }).observe(container);

  // When visible logical range changes (pan/zoom), recompute
...
/**
 * Debounced version of updateLookAheadLine for performance
 */
function debouncedUpdateLookahead() {
  if (updateLookaheadDebounceTimer) {
    clearTimeout(updateLookaheadDebounceTimer);
  }
  updateLookaheadDebounceTimer = setTimeout(() => {
    updateLookAheadLine();
  }, 50);
}

/**
 * Compute bars ahead from deal duration and current timeframe and position the overlay.
 * Call whenever: timeframe change, deal duration change, new candle update, chart pan/zoom.
 */
function updateLookAheadLine(dealDurationSeconds, currentTF) {
  if (!activeChart || !lookaheadOverlay) {
    console.warn("Cannot update lookahead line: chart or overlay not ready");
    return;
  }

  // Get from window globals if not provided
  if (dealDurationSeconds === undefined) {
    dealDurationSeconds = window.dealDurationInSeconds || 5;
  }
  if (currentTF === undefined) {
    currentTF = window.currentTimeframe || currentTimeframe || '5s';
  }

  // Determine how many bars ahead the chosen duration represents
  const tfSeconds = getTimeframeSeconds(currentTF);
  const barsAhead = Math.max(1, Math.round(dealDurationSeconds / tfSeconds));
  lookaheadBarsAhead = barsAhead;

  // console.log(`Lookahead: ${dealDurationSeconds}s duration, ${tfSeconds}s timeframe = ${barsAhead} bars ahead`);

  // Get visible logical range & chart pixel width
  const timeScale = activeChart.timeScale();
  const logical = timeScale.getVisibleLogicalRange();
  const container = document.getElementById('tradingview-widget');
  const chartWidth = container ? container.clientWidth : 0;

  if (!logical || typeof logical.to === 'undefined' || typeof logical.from === 'undefined' || chartWidth === 0) {
    lookaheadOffsetPx = 50;
    lookaheadOverlay.style.left = `${Math.max(0, chartWidth - lookaheadOffsetPx)}px`;
    console.log("Lookahead: Using fallback position");
    return;
  }

  const visibleBarCount = logical.to - logical.from;
  if (!visibleBarCount || visibleBarCount <= 0) {
    lookaheadOffsetPx = 50;
    lookaheadOverlay.style.left = `${Math.max(0, chartWidth - lookaheadOffsetPx)}px`;
    console.log("Lookahead: No visible bars, using fallback");
    return;
  }

  // pixel per visible bar
  const pxPerBar = chartWidth / visibleBarCount;
  lookaheadOffsetPx = Math.max(2, Math.round(pxPerBar * barsAhead));

  // place the overlay inside container from left (chartWidth - offset)
  const leftPos = Math.max(0, chartWidth - lookaheadOffsetPx);
  lookaheadOverlay.style.left = `${leftPos}px`;
  
  // console.log(`Lookahead positioned at ${leftPos}px (offset: ${lookaheadOffsetPx}px, pxPerBar: ${pxPerBar.toFixed(2)})`);
}

/**
 * Call this whenever chart is recreated
 */
function ensureLookAheadInitialized() {
  if (!lookaheadOverlay) {
    createLookAheadOverlay();
  } else {
    updateLookAheadLine();
  }
}
...
async function initTradingView() {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            // Subscribe to scroll/zoom events
            activeChart.timeScale().subscribeVisibleTimeRangeChange(() => {
        ...
        // Initialize lookahead overlay
        ensureLookAheadInitialized();
